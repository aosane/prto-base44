...
export default function LeadsSidebar({ activeTab, setActiveTab, onFilterCountChange }) {
  ...
  const [signalsExpanded, setSignalsExpanded] = useState(null);
      ...
      <div className="p-4 flex-1 overflow-y-auto">
        ...
        {activeTab === 'search' && (
          <>
            ...
            <div className="mt-6">
              <button
                onClick={() => setSignalsExpanded(signalsExpanded === 'signals' ? null : 'signals')}
                className="w-full flex items-center justify-between px-3 py-2 text-xs font-medium text-gray-400 uppercase mb-2 hover:text-gray-600"
              >
                <div className="flex items-center gap-2">
                ...
                {signalsExpanded === 'signals' ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
              </button>
