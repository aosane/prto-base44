      ...
      <main className="flex-1 overflow-y-auto bg-white">
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 overflow-auto px-6 py-4 relative">
            <div className={signalFilterActive && !unlocked ? 'blur-sm pointer-events-none select-none' : ''}>
              <LeadsTable />
            </div>
            {signalFilterActive && !unlocked && (
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="bg-white rounded-xl shadow-lg border border-gray-200 px-8 py-6 text-center max-w-sm mx-4">
                  <div className="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center mx-auto mb-3">
                    <Lock className="w-5 h-5 text-orange-500" />
                  </div>
                  <h3 className="text-sm font-semibold text-gray-900 mb-1">Ces leads coûtent des crédits</h3>
                  <p className="text-xs text-gray-500 mb-4">Les filtres Signals consomment <span className="font-medium text-orange-600">1 crédit par lead</span>. Sauvegardez dans une liste ou enrichissez pour débloquer.</p>
                  <div className="flex flex-col gap-2">
                    <button
                      onClick={() => setUnlocked(true)}
                      className="w-full px-4 py-2 bg-[#1C64F2] text-white text-sm font-medium rounded-lg hover:bg-[#1854cc] transition-colors flex items-center justify-center gap-2"
                    >
                      <Sparkles className="w-4 h-4" />
                      Enrich all (10 crédits)
                    </button>
                    <button
                      onClick={() => setUnlocked(true)}
                      className="w-full px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
                    >
                      <FolderPlus className="w-4 h-4" />
                      Add to list
                    </button>
